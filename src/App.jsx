...
const TRANSLATIONS = {
  ...
  bg: {
    ...
    deleteConfirmMsg:
      'Сигурни ли сте, че искате да изтриете това? Това не може да бъде отменено.',
    cancel: 'Отказ',
    delete: 'Изтрий',
  ...
  de: {
    ...
    cannotDelete:
      'Kategorie mit existierenden Transaktionen kann nicht gelöscht werden.',
  ...
  it: {
    ...
    deleteConfirmMsg:
      'Sei sicuro di voler eliminare? Non può essere annullato.',
    cancel: 'Annulla',
    delete: 'Elimina',
...
export default function App() {
  ...
  // New State for editing initial savings
  ...
  const handleImport = (e) => {
    ...
    try {
      ...
      if (data.transactions && data.categories) {
        if (
          window.confirm('This will overwrite your current data. Are you sure?')
        ) {
  ...
  // 2. Monthly Transactions (Filtered)
  ...
  const yearlyStats = useMemo(() => {
    ...
    const months = Array.from({ length: 12 }, (_, i) => ({
      name: new Date(year, i).toLocaleString(LOCALE_MAP[lang], {
        month: 'short',
      }),
      income: 0,
      expense: 0,
    ...
    transactions.forEach((t) => {
      ...
      if (tDate.getFullYear() === year) {
        if (t.type === 'income')
          months[tDate.getMonth()].income += Number(t.amount);
        else if (t.type === 'expense')
          months[tDate.getMonth()].expense += Number(t.amount);
      }
    });
  ...
  const NotificationToast = () => {
    ...
    return (
      <div
        className={`fixed top-4 left-1/2 -translate-x-1/2 z-[60] px-6 py-3 rounded-xl shadow-xl flex items-center gap-3 animate-fade-in ${
          ...
          {isEditingInitial ? (
            <div className="flex items-center gap-2 mb-6 mt-2">
              <input
                ...
                className="bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-2xl font-bold w-full max-w-[200px] focus:outline-none focus:ring-2 focus:ring-yellow-400"
                autoFocus
              />
              <button
                onClick={saveInitialSavings}
                className="p-3 bg-emerald-500 hover:bg-emerald-600 rounded-lg text-white"
              >
                <Save size={20} />
              </button>
              <button
            ...
            <p className="text-4xl md:text-5xl font-bold mb-6">
              {formatCurrency(globalStats.balance, LOCALE_MAP[lang])}
            </p>
          )}
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-4">
            <div>
              ...
              <p className="font-semibold text-emerald-400">
                {formatCurrency(globalStats.income, LOCALE_MAP[lang])}
              </p>
            </div>
            <div>
              <p className="text-slate-400 text-xs uppercase font-bold">
              ...
              <p className="font-semibold text-rose-400">
                {formatCurrency(globalStats.expenses, LOCALE_MAP[lang])}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <h2 className="text-3xl font-bold mt-1">
                {formatCurrency(stats.balance, LOCALE_MAP[lang])}
              </h2>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-slate-800">
                {formatCurrency(stats.income, LOCALE_MAP[lang])}
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-slate-800">
                {formatCurrency(stats.expenses, LOCALE_MAP[lang])}
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="space-y-4">
          ...
          {categoryStats.length === 0 ? (
            ...
            categoryStats.map((cat) => {
              const percentage =
                cat.limit > 0
                  ? Math.min((Math.max(0, cat.spent) / cat.limit) * 100, 100)
                  : 0;
                  ...
                  <div className="flex justify-between items-end mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`text-xs font-bold px-2 py-1 rounded-full text-white ${cat.color} inline-block`}
                        >
                          {cat.name}
                        </span>
                        {isSavings && (
                      ...
                      <div className="text-sm text-slate-500 mt-1">
                        {isSavings ? t('saved') : t('spent')}:{' '}
                        <span className="font-semibold text-slate-700">
                          {formatCurrency(cat.spent, LOCALE_MAP[lang])}
                        </span>
                        <span className="mx-1">/</span>
                        {isSavings ? t('goal') : t('limit')}:{' '}
                        {formatCurrency(cat.limit, LOCALE_MAP[lang])}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-2 h-10 rounded-full ${
                        cat?.color || 'bg-gray-300'
                      }`}
                    ></div>
                    <div>
                      <p className="font-medium text-slate-800">
                        {t.description || cat?.name || t('unknown')}
                      </p>
                      <p className="text-xs text-slate-400">
                        {new Date(t.date).toLocaleDateString(LOCALE_MAP[lang])}
                      </p>
                    </div>
                  </div>
                  <span
  ...
  const AnalysisView = () => {
    const maxVal = Math.max(
      ...yearlyStats.map((m) => Math.max(m.income, m.expense)),
      100
    );
      ...
      <div className="space-y-8 pb-24 md:pb-0 animate-fade-in">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold text-slate-800">
              {t('yearlyOverview')} ({currentDate.getFullYear()})
            </h3>
                ...
                >
                  ...
                  <span
                    className={`text-xs font-medium capitalize ${
                      month.name ===
                      getMonthName(currentDate, lang).substring(0, 3)
        ...
        <div>
          <h3 className="text-xl font-bold text-slate-800 mb-4 capitalize">
            {t('breakdown')} {getMonthName(currentDate, lang)}
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {categoryStats.length === 0 ? (
              <div className="col-span-full text-center p-8 text-slate-400 bg-slate-50 rounded-xl">
                {t('noRecords')}
              </div>
            ) : (
              categoryStats
                ...
                .map((cat) => (
                  <Card key={cat.id} className="flex items-center gap-4">
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center text-white text-lg font-bold shadow-sm ${cat.color}`}
                    >
                      {Math.round(
                        (Math.max(0, cat.spent) /
                          (stats.expenses + Math.max(0, stats.savings))) *
                          100
                      )}
                      %
                    </div>
      ...
      <div className="max-w-md mx-auto animate-fade-in pb-24 md:pb-0">
        ...
        <Card>
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {filteredCategories.map((cat) => (
                  <button
                    ...
                    className={`p-3 rounded-xl border-2 text-left transition-all ${
                      categoryId === cat.id
                        ? `border-${cat.color.replace('bg-', '')} bg-slate-50`
                        : 'border-transparent bg-slate-50 hover:bg-slate-100'
                    }`}
                  >
                    <div
                      className={`w-full h-1 rounded-full ${cat.color} mb-2`}
                    ></div>
                    <span
            ...
            >
              <CheckCircle size={20} />{' '}
              {isWithdrawal ? t('withdraw') : t('saveTransaction')}
            </Button>
          </form>
        </Card>
      </div>
          ...
          transactions
            ...
            .map((t) => {
              ...
              return (
                <Card
                  key={t.id}
                  className="flex justify-between items-center group hover:border-blue-200 transition-colors"
                >
                  <div className="flex items-center gap-4">
                    <div
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        {new Date(t.date).toLocaleDateString(LOCALE_MAP[lang])}{' '}
                        •{' '}
                        {new Date(t.date).toLocaleTimeString(LOCALE_MAP[lang], {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 capitalize">
                    {cat.type}{' '}
                    {(cat.type === 'expense' || cat.type === 'savings') &&
                    cat.limit > 0
                      ? `• ${
                          cat.type === 'savings' ? t('goal') : t('limit')
                        }: ${formatCurrency(cat.limit, LOCALE_MAP[lang])}`
                      : ''}
                  </p>
                </div>
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-blue-100">
                  ...
                  <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border border-slate-100 p-2 z-20 w-48 animate-fade-in">
                    {LANGUAGES.map((l) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-3 hover:bg-slate-50 ${
                          lang === l.code
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-6 py-4 pb-6 shadow-2xl z-40 flex justify-between items-center">
        ...
        <div className="relative -top-8">
          <button
            onClick={() => setActiveTab('add')}
            className={`w-14 h-14 rounded-full flex items-center justify-center text-white shadow-lg shadow-blue-200 transition-transform active:scale-95 ${
              activeTab === 'add' ? 'bg-slate-800' : 'bg-blue-600'
            }`}
          >
            <PlusCircle size={28} />
          </button>
